  ...
  document.querySelectorAll('.detalles-btn').forEach(button => {
    button.addEventListener('click', () => {
      ...
      if (solicitud) {
        ...
        } else if (solicitud.estado === 'Aprobado por Jefe') {
          estadoBadge = `<span class="badge badge-warning">Aprobado por Jefe</span>`;
        } else {
...
async function aprobarSolicitud(id) {
  Swal.fire({
    ...
    text: "Esta acción aprobará definitivamente la solicitud.",
    icon: 'question',
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`/aprobar-solicitud-RH/${id}`, {
          method: 'POST',
        ...
        if (response.ok) {
          Swal.fire({
            ...
            text: 'La solicitud ha sido aprobada definitivamente.',
            timer: 3000,
...
async function rechazarSolicitud(id, motivo) {
  try {
    const response = await fetch(`/rechazar-solicitud-RH/${id}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
function setupFilters() {
  document.querySelectorAll('#filtroEstado .dropdown-item').forEach(item => {
    item.addEventListener('click', (e) => {
      ...
      exportLink.href = `/export-aprobacion-RH?type=historico${filter !== 'all' ? '&estado=' + filter : ''}`;
    });
  });
}
